...
class DadjokesValidationException implements Exception {
  final List<String> errors;

  DadjokesValidationException(this.errors);

  @override
  String toString() => 'DadjokesValidationException: ${errors.join("; ")}';
}
...
/// Dad Jokes API client.
///
/// For more information, visit: https://apiverve.com/marketplace/dadjokes?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class DadjokesClient {
  ...
  DadjokesClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/dadjokes',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DadjokesValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DadjokesValidationException(errors);
    }
  }

  /// Execute a request to the Dad Jokes API.
  ///
  /// Parameters are validated before sending the request.
  Future<DadjokesResponse> execute() async {
    if (apiKey.isEmpty) {
      throw DadjokesException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DadjokesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DadjokesException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DadjokesException('Resource not found');
      } else {
        throw DadjokesException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DadjokesException) rethrow;
      throw DadjokesException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Dad Jokes API client.
class DadjokesException implements Exception {
  final String message;

  DadjokesException(this.message);

  @override
  String toString() => 'DadjokesException: $message';
}
